...
export const metadata = {
  title: "Chợ Nông Thủy Sản Biến Động 📈 — Waguri Bot",
  description: "Bảng tra cứu giá nông sản, thủy sản, khoáng sản biến động thời gian thực của Waguri Bot.",
};
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 py-12 md:py-16 space-y-10 z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white">
            Chợ Nông Thủy Sản <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-400 via-rose-300 to-amber-200">Biến Động</span>
          </h1>
          ...
          <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl glass-panel border border-pink-300/20 bg-pink-500/5">
            ...
            <div className="text-left">
              <p className="text-xs text-slate-400 font-medium">Lần biến động giá tiếp theo sau:</p>
                  ...
                  {catPrices.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          <h3 className="text-base font-bold text-white">{p.nameVi}</h3>
                          <p className="text-xs text-slate-400">Giá cơ sở: {fmt(p.basePrice)} VNĐ</p>
                        </div>
                        ...
                        <div className="pt-2 border-t border-slate-800/60 flex items-baseline justify-between">
                          ...
                          <span className="text-lg font-black text-amber-300">{fmt(p.currentPrice)} <span className="text-xs font-semibold text-slate-400">VNĐ</span></span>
                        </div>
                      </div>
                    );
                  })}
        ...
        <div className="glass-panel rounded-3xl p-8 text-center border border-dashed border-pink-300/25 space-y-4">
          ...
          <h3 className="text-xl font-extrabold text-white">Bán nông sản ngay trên Discord</h3>
          <p className="text-slate-400 text-sm max-w-lg mx-auto leading-relaxed">
            Dùng lệnh <code className="px-2 py-1 rounded bg-pink-500/20 text-pink-300 font-mono text-xs">/market sell [vật_phẩm] [số_lượng]</code> hoặc <code className="px-2 py-1 rounded bg-pink-500/20 text-pink-300 font-mono text-xs">/store sell</code> để chốt lời theo giá chợ biến động!
          </p>
          <div className="pt-2">
            <Link
              href="/commands"
